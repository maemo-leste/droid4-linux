					    ...
					    unsigned int code, int value)
{
	...
	if (type == EV_SYN && code == SYN_REPORT) {
		if (!buttons->queue.overrun &&
		    touchscreen_buttons_process_syn(
			    &buttons->queue, buttons->map,
			    buttons->buttons_idev) == 0) {
			touchscreen_buttons_resend_events(
						&buttons->queue,
		...
		touchscreen_buttons_resend_events(&buttons->queue,
						  buttons->filtered_ts_idev);
		buttons->queue.lastindex = 0;
		touchscreen_buttons_reset_buttons(buttons->map,
						  buttons->buttons_idev);
...
static void touchscreen_buttons_merge_capabilities(struct input_dev *target,
						   struct input_dev *source)
{
	unsigned int i;
...
static void merge_task_handler(struct work_struct *work)
{
	...
	buttons = container_of(work, struct touchscreen_buttons, merge_task);
	mutex_lock(&buttons->mutex);

	if (buttons->ts_handle && buttons->ts_handle->dev) {
		touchscreen_buttons_merge_capabilities(
					buttons->filtered_ts_idev,
...
static void close_task_handler(struct work_struct *work)
{
	...
	buttons = container_of(work, struct touchscreen_buttons, close_task);
...
static void open_task_handler(struct work_struct *work)
{
	...
	buttons = container_of(work, struct touchscreen_buttons, open_task);
	mutex_lock(&buttons->mutex);
...
static void touchscreen_buttons_remove(struct platform_device *pdev)
{
	struct touchscreen_buttons *buttons;
	struct input_handle *ts_handle;
