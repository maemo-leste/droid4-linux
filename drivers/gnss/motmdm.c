				    ...
				    const u8 *buf, int len)
{
	...
	ret = serdev_device_write_buf(ddata->serdev, cmd, cmdlen);
	if (ret < 0)
		goto out_unlock;
...
static size_t motmdm_gnss_receive_data(struct serdev_device *serdev,
...
{
	struct motmdm_gnss_data *ddata = gnss_get_drvdata(gdev);
	struct serdev_device *serdev = ddata->serdev;
	int error;
...
static int motmdm_gnss_write_raw(struct gnss_device *gdev,
				 ...
				 size_t count)
{
	struct motmdm_gnss_data *data = gnss_get_drvdata(gdev);
