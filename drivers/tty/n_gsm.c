...
static size_t gsd_receive_buf(struct serdev_device *serdev, const u8 *data,
			      size_t count)
{
...
int gsm_serdev_register_tty_port(struct gsm_serdev *gsd, int line)
{
	...
	dlci = gsd_dlci_get(gsd, ops->line, false);
	index = mux_num_to_base(gsd->gsm) + ops->line;
	parent = &gsd->serdev->dev;
	for_each_available_child_of_node(parent->of_node, node) {
		struct platform_device_info devinfo = {};
		struct platform_device *pdev;
		unsigned int line;

		if (of_property_read_u32(node, "reg", &line) ||
		    line != ops->line) {
			continue;
		}

		devinfo.name = kasprintf(GFP_KERNEL, "gsm-mux-%d", line);
		devinfo.parent = parent;

		pdev = platform_device_register_full(&devinfo);
		pdev->dev.of_node = node;
		dlci->pdev = pdev;
		dev = tty_port_register_device_attr_serdev(&dlci->port,
			gsm_tty_driver, index, &pdev->dev, &pdev->dev, NULL, NULL);

		/*
		 * DLCI tty ports registered for serdev are kernel-consumed
		 * transport endpoints and are not exposed to userspace. Mark
		 * the tty port as KOPENED so tty hangup accounting accepts the
		 * serdev-held tty reference during DLCI teardown.
		 */
		if (!IS_ERR(dev))
			tty_port_set_kopened(&dlci->port, true);

		break;
	}

	if (!dev)
		dev = tty_register_device(gsm_tty_driver, index, NULL);
...
void gsm_serdev_unregister_tty_port(struct gsm_serdev *gsd, int line)
{
	...
	tty_port_unregister_device(&dlci->port, gsm_tty_driver, base + line);

	if (dlci->pdev)
		platform_device_unregister(dlci->pdev);
...
int gsm_serdev_device_open(struct gsm_serdev *gsd)
{
	...
	return serdev_device_open(gsd->serdev);
}
EXPORT_SYMBOL_GPL(gsm_serdev_device_open);
...
int gsm_serdev_register_device(struct gsm_serdev *gsd)
{
	...
	if (WARN(!gsd || !gsd->serdev || !gsd->serdev->ops || !gsd->output,
